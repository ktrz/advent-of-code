...
export const toHexadecimal = (value: string): string =>
  value
    .trim()
    .split('')
...
export const getPacket = (stream: string) => {
...
export const sumVersions = (packet: Packet): number => {
  switch (packet.packetType) {
    ...
    default:
      return packet.version + packet.subPackets.map(sumVersions).reduce(sum, 0)
  }
}

export const calculatePacket = (packet: Packet): number => {
  switch (packet.packetType) {
    case 'Literal':
      return packet.value
    default:
