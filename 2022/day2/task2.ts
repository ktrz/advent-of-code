import { input } from './input2'

const choices = {
  Rock: 'Rock',
  Paper: 'Paper',
  Scissors: 'Scissors',
} as const

type Choices = typeof choices
type ChoicesKeys = keyof Choices
type ChoicesValues = Choices[ChoicesKeys]

const choicesValue: Record<ChoicesValues, number> = {
  Rock: 1,
  Paper: 2,
  Scissors: 3,
}

const symbolToChoiceMap: Record<'A' | 'B' | 'C', ChoicesValues> = {
  A: choices.Rock,
  B: choices.Paper,
  C: choices.Scissors,
}

const winMap: Record<ChoicesValues, ChoicesValues> = {
  [choices.Rock]: choices.Scissors,
  [choices.Paper]: choices.Rock,
  [choices.Scissors]: choices.Paper,
}

const looseMap: Record<ChoicesValues, ChoicesValues> = {
  [choices.Scissors]: choices.Rock,
  [choices.Rock]: choices.Paper,
  [choices.Paper]: choices.Scissors,
}

function isElfSymbol(elfSymbol: string): elfSymbol is 'A' | 'B' | 'C' {
  return (['A', 'B', 'C'] as const).includes(elfSymbol as any)
}

function isResult(mySymbol: string): mySymbol is 'X' | 'Y' | 'Z' {
  return (['X', 'Y', 'Z'] as const).includes(mySymbol as any)
}

function getCounterChoice(
  elfChoice: ChoicesValues,
  result: 'X' | 'Y' | 'Z',
): ChoicesValues {
  return result === 'X'
    ? winMap[elfChoice]
    : result === 'Y'
    ? elfChoice
    : looseMap[elfChoice]
}

const solution = (inputValue: string) => {
  return inputValue
    .trim()
    .split('\n')
    .map((line) => {
      const values = line.split(' ')
      const [elfSymbol, result] = values
      if (!isElfSymbol(elfSymbol)) {
        throw new Error(`invalid input for elf: ${elfSymbol}`)
      }
      if (!isResult(result)) {
        throw new Error(`invalid input for result: ${result}`)
      }
      return [elfSymbol, result] as const
    })
    .map(([elfSymbol, result]) => {
      const elf = symbolToChoiceMap[elfSymbol]
      const my = getCounterChoice(elf, result)
      my
      return {elf, my}
    })
    .reduce((score, { elf, my }) => {
      const choiceScore = choicesValue[my]
      const outcomeScore = winMap[my] === elf ? 6 : my === elf ? 3 : 0

      const newScore = choiceScore + outcomeScore
      return score + newScore
    }, 0)
}
console.log(solution(input)) // ?
